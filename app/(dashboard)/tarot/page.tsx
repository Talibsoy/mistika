...
export default function TarotPage() {
  ...
  const [shuffled] = useState(() => [...TAROT_CARDS].sort(() => Math.random() - 0.5).slice(0, 12));
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
        {positions.map((pos, i) => (
          <div key={pos} style={{ flex: 1, minWidth: 140, background: "#111", border: `1px solid ${selected[i] ? "#d4af37" : "#222"}`, borderRadius: 12, padding: "16px 12px", textAlign: "center" }}>
            <div style={{ color: "#555", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>{pos}</div>
            <div style={{ color: selected[i] ? "#d4af37" : "#333", fontSize: 14, fontWeight: 600 }}>
              {selected[i] || "—"}
            </div>
          </div>
        ))}
        ...
        {shuffled.map((card) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              style={{
                background: isSelected ? "rgba(212,175,55,0.15)" : "#111",
            ...
            >
              {isSelected && (
                <div style={{ position: "absolute", top: 6, right: 8, width: 18, height: 18, borderRadius: "50%", background: "#d4af37", color: "#0a0a0a", fontSize: 10, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {idx + 1}
                </div>
              )}
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <textarea value={sual} onChange={e => setSual(e.target.value)} rows={2}
      ...
      <button onClick={handleSubmit} disabled={selected.length !== 3 || loading}
        style={{ background: selected.length === 3 ? "linear-gradient(135deg, #dc2626, #b91c1c)" : "#222", color: selected.length === 3 ? "white" : "#555", border: "none", padding: "13px 32px", borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: selected.length === 3 ? "pointer" : "not-allowed", transition: "all 0.3s" }}>
